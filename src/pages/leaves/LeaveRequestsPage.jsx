  ...
  const columns = useMemo(
    () => [
      ...
      columnHelper.accessor('reason', {
        header: 'Reason',
        cell: ({ getValue }) => (
          <span className="max-w-xs truncate block text-slate-400">
            {getValue() || '—'}
          </span>
        ),
          ...
          if (r.status !== 'pending') {
            ...
            return (
              <span
                ...
                className="text-rose-400 text-xs cursor-help"
              >
                Rejected
              </span>
            )
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="bg-amber-500/10 border border-amber-500/20 rounded-xl px-4 py-3">
          <p className="text-amber-400 font-semibold">{pendingCount}</p>
          <p className="text-slate-400 text-xs">Pending</p>
        </div>
        <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-xl px-4 py-3">
          <p className="text-emerald-400 font-semibold">{approvedCount}</p>
          <p className="text-slate-400 text-xs">Approved</p>
        </div>
        <div className="bg-rose-500/10 border border-rose-500/20 rounded-xl px-4 py-3">
          <p className="text-rose-400 font-semibold">{rejectedCount}</p>
