...
export function DeviceChangeRequests() {
  ...
  return (
    <div className="space-y-6 flex-1 w-full max-w-full overflow-hidden p-6 mx-auto">
      ...
      <div className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-2xl overflow-hidden w-full">
        {loading ? (
          [...Array(6)].map((_, i) => (
            <div
              key={i}
            ...
            >
              {[...Array(6)].map((_, j) => (
                <div
                  key={j}
                  className="h-6 flex-1 bg-gray-100 dark:bg-slate-700/50 rounded animate-pulse"
                />
              ))}
            </div>
          ))
              ...
              <table className="w-full">
                <thead>
                  {table.getHeaderGroups().map((headerGroup) => (
                    <tr
                      key={headerGroup.id}
                      className="border-b border-gray-200 dark:border-slate-800 bg-gray-50 dark:bg-slate-900/50"
                    >
                      {headerGroup.headers.map((header) => (
                        <th
                          key={header.id}
                          className={`text-left text-xs font-semibold uppercase tracking-wider text-gray-600 dark:text-slate-500 px-4 py-3 whitespace-nowrap ${
                ...
                <tbody>
                  {table.getRowModel().rows.map((row) => (
                    <tr
                      key={row.id}
                      className="border-b border-gray-200 dark:border-slate-800/80 hover:bg-gray-100 dark:hover:bg-slate-800/40 transition-colors duration-150"
                    >
                      {row.getVisibleCells().map((cell) => (
                        <td
                          key={cell.id}
                          className={`px-4 py-3 text-gray-900 dark:text-white text-sm align-middle ${
      ...
      >
        {rejectTarget && (
          <div className="space-y-4">
            <div className="flex items-center gap-3 p-3 bg-gray-100 dark:bg-slate-800/50 rounded-xl">
              ...
              <div>
                <p className="font-medium text-gray-900 dark:text-white">
                  {rejectTarget.user?.username || "Unknown"}
                </p>
