...
export function UserLocationsPage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        // Map fullName to name for display compatibility
        const mapped = usersData.map((m) => ({
          ...m,
  ...
  useEffect(() => {
    ...
    getUserLocation(selectedId)
      .then((res) => {
        // API returns { success, data } with attendance profile data
        const d = res.data.data || res.data;
        setLocationExists(true);
        // Map new field names: officeLatitude, officeLongitude, officeRadiusMeters
        setLatitude(String(d.officeLatitude ?? ""));
        setLongitude(String(d.officeLongitude ?? ""));
        setRadius(String(d.officeRadiusMeters ?? ""));
        setUpdatedAt(d.updatedAt ?? null);
        // updatedBy is a relation object with { id, fullName }
        setUpdatedBy(d.updatedBy?.fullName ?? d.updatedByUserId ?? null);
  ...
  const handleSave = async (e) => {
    ...
    setError(null);
    try {
      // Use new field names: officeLatitude, officeLongitude, officeRadiusMeters
      await setUserLocation(selectedId, {
        officeLatitude: lat,
        officeLongitude: lng,
        officeRadiusMeters: rad,
      });
      setLocationExists(true);
      ...
      setSuccess(
        `Location updated for ${currentMember?.name || "selected member"}.`,
      );
  ...
  return (
    <div className="flex flex-col md:flex-row h-[calc(100vh-8rem)] md:h-[calc(100vh-6rem)] gap-0 -m-4 lg:-m-8">
      <aside className="w-full md:w-80 border-r border-gray-200 dark:border-slate-800 bg-white dark:bg-slate-900/50 flex flex-col shrink-0">
        <div className="p-4 border-b border-gray-200 dark:border-slate-800">
          <div className="relative">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-500 dark:text-slate-200" />
            <input
              type="text"
        ...
        <div className="flex-1 overflow-y-auto scrollbar-minimal">
          {loadingMembers ? (
            [...Array(5)].map((_, i) => (
              <div
                key={i}
                className="p-4 border-b border-gray-200 dark:border-slate-800 animate-pulse flex items-center gap-3"
              >
                ...
                <div className="flex-1">
                  ...
                  <div className="h-3 w-32 bg-gray-100 dark:bg-slate-700 rounded mt-2" />
                </div>
              </div>
            ))
            ...
            filteredMembers.map((m) => (
              <button
                ...
                className={`
                  w-full p-4 flex items-center gap-3 text-left border-b border-gray-200 dark:border-slate-800
                  hover:bg-gray-100 dark:hover:bg-slate-800/50 transition-colors duration-150
                  ${selectedId === m.id ? "bg-indigo-600/10 border-l-2 border-l-indigo-500 border-b-gray-200 dark:border-b-slate-800" : ""}
                `}
              >
      ...
      <div className="flex-1 overflow-y-auto p-4 lg:p-8">
        ...
        {!selectedId ? (
          <div className="flex flex-col items-center justify-center py-24 text-center">
            <div className="w-16 h-16 bg-gray-100 dark:bg-slate-800 rounded-3xl flex items-center justify-center mb-4">
              <MapPin className="w-8 h-8 text-gray-600 dark:text-slate-600" />
            </div>
